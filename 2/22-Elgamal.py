"""Пусть М — исходное сообщение, состоящее из последовательных
шифрвеличин M i
Шифрование:
1. Выбирается большое простое число p, p > M i.
2. Выбираются числа x и g так, что 1 < x < p, 1 < g < p.
3. Вычисляется
Таким образом, открытые ключи: p, g, y. Секретный ключ: x.
4. Выбираются случайные секретные числа k i (рандомизаторы), взаимно
простые с функцией Эйлера от числа р (φ(p) = p − 1):
(k i , φ(p)) = 1.
5. Вычисляется для каждой шифрвеличины M i :
a i = g ki (mod p)
b i = y ki M i (mod p)
Пара чисел a и b является шифробозначением.
Последовательность a ib i образует шифртекст.
Длина шифртекста получается длинней открытого текста вдвое.
Расшифрование:
Для расшифрования a и b, вычисляется:
M i ≡ b i/ a i x (mod p)
Т.е. решается сравнение относительно М:
Для этого надо избавиться от знаменателя: умножаем левую и правую часть
сравнения на а х и решаем полученное сравнение относительно M, пользуясь
