...
def main():
    ...
    print_kv("Сообщение", message.hex())
    print_kv("Ключ", key.hex())

    ciphertext = AES(key).encrypt_block(message)
    print_kv("Шифровка по стандарту", ciphertext.hex())
    ...
    enc = AES(key).encrypt_cbc(text_test.encode(), iv)

    dec = AES(key).decrypt_cbc(enc, iv)

    print_kv("Шифровка Тест", enc.hex())
