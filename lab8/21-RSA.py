"""
Берутся два очень больших простых числа P и Q и находятся произведение
простых чисел
N=P×Q
и функция Эйлера от этого произведения
φ(N)=(P-1)×(Q-1).

Выбирается случайное целое число E: 1 < Е < φ(N) и взаимно простое с
φ(N), и вычисляется
D=(1 MOD φ(N))/E.
E и N публикуются как открытый ключ, D сохраняется в тайне.

Шифрование:
Если M — сообщение, то шифртекст C i получается последовательным
шифрованием каждой шифрвеличины M i возведением ее в степень E по
модулю N:
C i = M i E MOD N.

Расшифрование:
Получатель расшифровывает сообщение, возводя последовательно С i в
степень D по модулю N:
M i = C i D MOD N.
"""
